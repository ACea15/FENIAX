...
def dfield(description, **kwargs):
    ...
    metadata = {"description": description, "options": options, "yaml_save": yaml_save}
...
def dump_inputs(
    data: dict[str : list[Any, str]], indent: int = 0, with_comments: bool = True
):
    # if ind == 0:
    data = CommentedMap(data)
    for k, v in data.items():
        if isinstance(v, dict):
            data[k] = dump_inputs(v, indent=indent + 1, with_comments=with_comments)
        else:
            data[k] = v[0]
            # data.yaml_add_eol_comment(v[1], k)
            if with_comments:
                data.yaml_set_comment_before_after_key(
                    k, before=v[1], indent=2 * indent
                )
    return data


def dump_yaml(
    file_out: str | pathlib.Path,
...
if __name__ == "__main__":
    comp_conn = dict(c1=["c2", "c3", "c5"], c2=None, c3=["c4"], c4=[], c5=None)
    ...
    comp_conn2 = dict(c1=["c2", "c3", "c5"], c2=None, c3=["c4"], c4=[], c5=None, c6=[])
    ...
    prevnodes2 = compute_prevnode(
        components_range2, component_nodes2, component_father2
    )
