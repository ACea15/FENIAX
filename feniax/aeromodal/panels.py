...
def caero2grid(components, caeros):
    ...
    for i, k in enumerate(components):
        Xcorners = caero2corners(
            caeros[i]["p1"], caeros[i]["p4"], caeros[i]["x12"], caeros[i]["x43"]
        )
        leading_edge, trailing_edge = lt_points(
            caeros[i]["nspan"], Xcorners[0], Xcorners[1], Xcorners[2], Xcorners[3]
        )
...
def build_gridmesh(components, save_file=None, save_dir=None) -> dict:
    grid = dict()
    for k, v in components.items():
        grid[k] = dict()
        _points, _cells = build_pyvista(
            v["leading_edge"], v["trailing_edge"], v["chordwise_points"]
        )
        ...
        if save_file is not None:
            ...
            mesh.save(file_path / f"{save_file}_{k}.ply", binary=False)
