...
def home():
    # st.write("# Welcome to FENIAX.✈️")

    # st.sidebar.success("Select a demo above.")

    st.markdown(
        """
        FENIAX.is an aeroelastic toolbox written and parallelized in Python,
...
def sys_states(solsys):
    ...
    if statei is not None:
        ...
        fig = uplotly.lines2d(
            ...
            q[:, statei],
            None,
            dict(name="NMROM", mode=mode, line=dict(color="navy")),
    ...
    with st.expander("Select multiple states"):
        ...
        if sinput is not None and len(sinput) > 0:
            ...
            fig2 = uplotly.lines2d(
                t,
                q[:, linput[0]],
                None,
                dict(name=f"State {linput[0]}", line=dict(color="navy"), mode=mode),
                dict(title="qs"),
            )
            for li in linput[1:]:
                fig2 = uplotly.lines2d(
                    t, q[:, li], fig2, dict(name=f"State {li}", mode=mode)
                )
...
def sys_X(X, solsys, label="X"):
    ...
    if nodei is not None:
        fig = uplotly.lines2d(
            ...
            X[:, componenti, nodei],
            None,
            dict(name="NMROM", line=dict(color="navy"), mode=mode),
                ...
                for ni in l_ninput:
                    for ci in l_cinput:
                        fig2 = uplotly.lines2d(
                            ...
                            X[:, ci, ni],
                            fig2,
                            dict(name=f"Node {ni}, component {ci}", mode=mode),
                        )
                st.plotly_chart(fig2, use_container_width=True)
...
def sys_3Dconfiguration(solsys, config, ti=None, labels=None, settings=None):
    ...
    if settings is None:
        settings = {}
    fig = uplotly.render3d_multi(icomp, labels, **settings)
    fig.update_layout(margin=dict(autoexpand=True, l=0, r=0, t=0, b=0))
...
def sys_3Dconfiguration_ti(solsys, config, settings=None):
    ...
    if settings is None:
        settings = {}
    # breakpoint()
    fig = uplotly.render3d_struct(icomp, label, **settings)
    fig.update_layout(
        showlegend=False,
        width=1000,
        height=900,
    ...
    fig.update_traces(line=dict(width=1.2, color="navy"), marker=dict(size=1.5))
...
def sys_3Dconfiguration0(config):
    ...
    icomp = putils.IntrinsicStructComponent(config.fem)
    # breakpoint()
    fig = uplotly.render3d_struct(
        icomp,
        label="ref1",
        scatter_settings=[dict(customdata=ti, hovertemplate=ti) for ti in template],
        update_traces=dict(line=dict(width=1.2, color="navy"), marker=dict(size=1.5)),
    )
    ...
    fig.update_layout(
        autosize=False,
        margin=dict(autoexpand=False, l=0, r=1.5, t=1.5, b=0),
        showlegend=False,
        width=1000,
        height=900,
    )
