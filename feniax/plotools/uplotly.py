...
class ScatterSettings:
    ...
    )  # dict(color=colors[i], width=line_size[i])
    marker: dict = field(
        default_factory=lambda: {}
    )  # dict(color=colors[i], size=mode_size[i])
...
def lines2d(
    x, y, fig=None, scatter_settings=None, update_layout=None, update_traces=None
):
    if fig is None:
        fig = go.Figure()
...
def lines3d(
    x, y, z, fig=None, scatter_settings=None, update_layout=None, update_traces=None
):
    if fig is None:
        fig = go.Figure()
    if scatter_settings is None:
...
def iterate_lines2d(
    x, y, scatter_settings=None, update_layout=None, update_traces=None, fig=None
):
    if scatter_settings is None:
    ...
    for i, xi in enumerate(x):
        if i < len(x) - 1:
            fig = lines2d(xi, y[i], fig=fig, scatter_settings=scatter_settings[i])
        else:
            fig = lines2d(
                xi,
                y[i],
...
def iterate_lines3d(
    x, y, z, scatter_settings=None, update_layout=None, update_traces=None
):
    fig = None
    for i, xi in enumerate(x):
        if i < len(x) - 1:
            fig = lines2d(xi, y[i], z[i], fig=fig, scatter_settings=scatter_settings[i])
        else:
            fig = lines2d(
                xi,
                y[i],
...
def render2d_struct(
    structcomp: putils.IntrinsicStructComponent,
    label=None,
    ...
    for i, v in enumerate(structure):
        ...
        if i < num_components - 1:
            fig = lines2d(
                v[:, xaxis], v[:, yaxis], fig=fig, scatter_settings=scsettings
            )
        else:
            fig = lines2d(
                v[:, xaxis],
...
def render3d_struct(
    ...
    for i, v in enumerate(structure):
        ...
        if i < num_components - 1:
            fig = lines3d(
                v[:, 0], v[:, 1], v[:, 2], fig=fig, scatter_settings=scsettings
            )
        else:
            fig = lines3d(
                v[:, 0],
...
def render3d_multi(
    ...
    for i, li in enumerate(labels):
        if isinstance(scatter_settings, list):
            ...
            scsettings = scatter_settings
        fig = render3d_struct(
            structcomp, li, scsettings, update_layout, update_traces, fig
        )
    return fig
...
def render2d_multi(
    ...
    for i, li in enumerate(labels):
        if isinstance(scatter_settings, list):
            ...
            scsettings = scatter_settings
        fig = render2d_struct(
            structcomp, li, scsettings, update_layout, update_traces, fig, xaxis, yaxis
        )
    return fig
