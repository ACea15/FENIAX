...
import numpy, struct
from feniax.plotools.nastranvtk.GenericVTKWriter.Common import bincalc
        ...
        if op2 is not None:
            if len(op2.eigenvectors.keys()) > 0:
                ...
                if hasattr(op2.eigenvectors[1], "mode_cycle") and not (
                    isinstance(op2.eigenvectors[1].mode_cycle, int)
                ):
                    cycles = op2.eigenvectors[1].mode_cycle
                else:
                    cycles = numpy.sqrt(numpy.real(numpy.array(eigvals))) / (
                        2.0 * numpy.pi
                    )
