...
import copy
import pathlib
import feniax.plotools.nastranvtk.bdf2vtk as bdf2vtk
...
def vtkModes_fromop2(
        bdf_file, op2_file, scale=100.0, modes2plot=None, size_card=8, write_path=None, plot_ref=True
):
    bdfile = pathlib.Path(bdf_file)
...
def vtkSol_op2Modes(
    bdf_file, op2_file, q, label="Sol", size_card=8, write_path=None, write_ref=True
):
    num_modes = len(q)
    bdfile = pathlib.Path(bdf_file)
    mbdf = BDF()
...
class DefBdf:
    ...
    def get_nodes(self, sort=True):
        if sort:
            nodes = [ni.get_position() for ni in self.mbdf.Nodes(self.sorted_nodeids)]
        else:
...
if __name__ == "__main__":
    vtk_fromop2(
        "/media/acea/work/projects/FENIAX/examples/SailPlane/NASTRAN/SOL103/run_cao.bdf",
        "/media/acea/work/projects/FENIAX/examples/SailPlane/NASTRAN/SOL103/run_cao.op2",
    )
