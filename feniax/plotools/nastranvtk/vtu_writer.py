...
import sys
import feniax.plotools.nastranvtk.GenericVTKWriter.VTUWriter as VTUWriter


import numpy, os

import feniax.plotools.nastranvtk.elem_sorter as elem_sorter
import feniax.plotools.nastranvtk.elem_data as elem_data
import feniax.plotools.nastranvtk.common as common
import feniax.plotools.nastranvtk.point_data as point_data
...
def writeVTU(
    filepath, fem, op2, elemName, nodes, nodeIDs, datatype="binary", modes2keep=None
):
    ...
    if elemName in ["RBE1", "RBE2", "RBE3", "RBAR", "RBAR1"]:
        elems = [
            value for key, value in fem.rigid_elements.items() if value.type == elemName
        ]
        ...
        elems = [
            value for key, value in fem.pseudoElements.items() if value.type == elemName
        ]
    ...
    if len(elems) > 0:
        nelems, elemtable, node_in_group, nnodes_in_group, elemIDs = (
            elem_sorter.sort_elems(fem, elemName, elems, nodes, nodeIDs)
        )
        point_data = {}
        cell_data = {}
        ...
        for i in range(len(elemtable)):
            for j in range((len(elemtable[i]))):
                if node_in_group[elemtable[i][j]] != -1:
                    ...
                    raise RuntimeError(
                        "Something wrong in connectivity calculation. This node is not in the group but is in the connectivity table"
                    )
        ...
        if op2 is not None:
            if len(op2.eigenvectors.keys()) > 0:
                ...
                if hasattr(op2.eigenvectors[1], "mode_cycle") and not (
                    isinstance(op2.eigenvectors[1].mode_cycle, float)
                ):
                    cycles = op2.eigenvectors[1].mode_cycle
                else:
                    cycles = numpy.sqrt(numpy.abs(numpy.array(eigvals))) / (
                        2.0 * numpy.pi
                    )
                ...
                for i, eigval in enumerate(eigvals):
                    if i == 0:
                        ...
                        EigvecNodeIDs = [
                            x[0] for x in op2.eigenvectors[1].node_gridtype
                        ]
                        ...
                        if len(EigvecNodeIDs) == len(nodeIDs):
                            ...
                            nodeindexes = numpy.searchsorted(
                                nodeIDs, EigvecNodeIDs
                            )  # Index in global nodeID list
                    ...
                    if modes2keep is None or i + 1 in modes2keep:
                        ...
                        for inodeID in range(
                            len(EigvecNodeIDs)
                        ):  # Index in eigenvector table
                            if node_in_group[nodeindexes[inodeID]] != -1:
                                ...
                                if isinstance(outcoord, int) and outcoord == 0:
                                    point_data[name][ielemnode, :] = op2.eigenvectors[
                                        1
                                    ].data[i, inodeID, 0:3]
                                    ...
                                    point_data[name][ielemnode, :] = (
                                        outcoord.transform_vector_to_global(
                                            op2.eigenvectors[1].data[i, inodeID, 0:3]
                                        )
                                    )
        ...
        VTUWriter.WriteVTU(
            vtufilename, points, elemtable, point_data, cell_data, datatype=datatype
        )
