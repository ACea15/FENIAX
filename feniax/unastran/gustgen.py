...
class Gust4Nastran:
    def __init__(
        ...
        self.N_step_T = int(
            self.T_max / self.delta_T
        )  # Number of time steps (3*real_time_step)
        ...
        if 0.0 <= self.h <= 4575.0:
            self.U_ref = -0.00080052 * self.h + 17.07
        if 4575.0 <= self.h <= 18288.0:
        ...
        for i in range(len(H)):
            ...
            if self.Type == "eas":
                ...
                self.rho = rho_0
            self.delta_T_gust.append(2.0 * self.H[i] / self.U_inf)
    ...
    def gust_family(self, write_gust, write_subcase):
        ...
        for i in range(len(self.H)):
            ...
            Gust_Card_file.write(
                "GUST".ljust(8)
                + str(int((i + 1) * 10)).ljust(8)
                + str(int(i + 1)).ljust(8)
                + ((str("%.7f" % float(1.0 / self.U_inf))).lstrip("0")).ljust(8)
                + "0.".ljust(8)
                + ("%.7s" % str(float(self.U_inf))).ljust(8)
            )
            Gust_Card_file.write("\n")
                ...
                for col in range(5):
                    if col == 0:
                        ...
                        if counter_t <= self.N_step_T:
                            ...
                            delta_T_gust = (
                                2.0 * self.H[i] / self.U_inf
                            )  # Time to get passed the gust
                            if time <= delta_T_gust:
                                U = (
                                    ...
                                    * (
                                        1
                                        - np.cos(np.pi * self.U_inf * time / self.H[i])
                                    )
                                )
                            ...
                            if (
                                time > self.T_max / 3
                                and time <= self.T_max / 3 + delta_T_gust
                            ):
                                U = (
                                    -2
                                    * self.U_ds[i]
                            ...
                            if (
                                time > self.T_max / 3 + delta_T_gust
                                and time <= self.T_max * 2 / 3
                            ):
                                U = 0.0
                            if (
                                time > self.T_max * 2 / 3
                                and time <= self.T_max * 2 / 3 + delta_T_gust
                            ):
                                U = (
                                    self.U_ds[i]
                                    / 2
    ...
    def gust_setup(self, setup_file):
        ...
        Gust_setup_file.write(
            "PARAM   Q".ljust(16) + ("%.7s" % str(float(self.q_dyn))).ljust(8)
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "PARAM   MACH".ljust(16) + ("%.7s" % str(float(self.M))).ljust(8)
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "AERO            ".ljust(16)
            + ("%.7s" % str(float(self.U_inf))).ljust(8)
            + str(self.chord).ljust(8)
            + ("%.7s" % str(float(self.rho))).ljust(8)
            + "0       0 ".ljust(24)
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            ...
            + "930".ljust(8)
            + "0.".ljust(8)
            + ("%.7s" % str(float(self.delta_f))).ljust(8)
            + ("%.7s" % str(self.N_step_freq)).ljust(8)
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "TSTEP".ljust(8)
            + "940".ljust(8)
            + ("%.7s" % str(int((self.N_step_T / 3)))).ljust(8)
            + ("%.7s" % str(float(self.delta_T))).ljust(8)
            + ("%.7s" % str(1)).ljust(8)
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
        ...
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "        0.00001 0.0001  0.001   0.01    0.015   0.02    0.03    0.04"
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write("MKAERO1".ljust(8) + ("%.7s" % str(self.M)).ljust(8))
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "        0.05    0.06    0.07    0.08    0.1     0.13    0.16    0.19"
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write("MKAERO1".ljust(8) + ("%.7s" % str(self.M)).ljust(8))
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "        0.23    0.28    0.33    0.38    0.43    0.48    0.53    0.59"
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write("MKAERO1".ljust(8) + ("%.7s" % str(self.M)).ljust(8))
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "        0.65    0.71    0.77    0.84    0.91    0.98    1.05    1.12"
        )
        Gust_setup_file.write("\n")
        Gust_setup_file.write("MKAERO1".ljust(8) + ("%.7s" % str(self.M)).ljust(8))
        Gust_setup_file.write("\n")
        Gust_setup_file.write(
            "        1.2     1.3     1.4     1.5     1.6     1.75    1.85    2.00"
        )
        Gust_setup_file.write("\n")

        Gust_setup_file.close()

        Gust_setup_file = open(
            "." + setup_file.split(".")[-2] + "_var." + setup_file.split(".")[-1], "w"
        )
