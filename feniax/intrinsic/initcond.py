...
def init_X1(modes, X1i: jnp.ndarray, *args, **kwags) -> jnp.ndarray:
    phi1 = modes.phi1
...
def init_X2(modes, X2i: jnp.ndarray, *args, **kwags) -> jnp.ndarray:
    phi2 = modes.phi2
...
mapper = dict(
    velocity=init_X1,  # given a velocity field, and phi1, output q10s
    force=init_X2,
    displacement=None,
)  # Placeholder
...
class Container:
    @staticmethod
    def axial_parabolic(
        x0: jnp.array, L: float, fem: intrinsicmodal.Dfem, *args, **kwags
    ):
        X0 = fem.X[0]
        f = jax.vmap(
            lambda Xi: (jnp.linalg.norm(Xi - X0) / L) ** 2, in_axes=0, out_axes=0
        )
        s = f(fem.X)
        s = s.reshape((len(s), 1))
