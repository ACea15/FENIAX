...
def compute_eigs(
    Ka: jnp.ndarray, Ma: jnp.ndarray, num_modes: int, *args, **kwargs
...
def compute_eigs_pass(num_modes, eigenvals, eigenvecs, *args, **kwargs):
    reduced_eigenvals = eigenvals[:num_modes]
    reduced_eigenvecs = eigenvecs[:, :num_modes]
    return reduced_eigenvals, reduced_eigenvecs
...
def shapes(
    ...
    omega = jnp.where(eigenvals < config.fem.eig_cutoff, 0.0, jnp.sqrt(eigenvals))
    ...
    moments_force = contraction(
        _moments_force, config.fem.Mload_paths, precision
    )  # (Nmx6xNn)
    ...
    phi1_diff = jnp.tensordot(phi1, config.fem.Mdiff, axes=(2, 0), precision=precision)
...
def check_alphas(phi1, psi1, phi2l, psi2l, X_xdelta, tolerance, *args, **kwargs):
    alpha1 = couplings.f_alpha1(phi1, psi1)
    alpha2 = couplings.f_alpha2(phi2l, psi2l, X_xdelta)
    alpha2_diagonal = alpha2.diagonal()
    # filter for rigid-body modes
    alpha2d_filtered = jnp.where(jnp.abs(alpha2_diagonal) > 1e-4, alpha2_diagonal, 1.0)
    alpha2_new = jnp.fill_diagonal(alpha2, alpha2d_filtered, inplace=False)
    num_modes = len(alpha1)
    Inm = jnp.eye(num_modes)
    assert jnp.allclose(
        alpha1, Inm, **tolerance
    ), f"Alpha1 not equal to Identity: Alpha1: {alpha1}"
    assert jnp.allclose(
        alpha2_new, Inm, **tolerance
...
def ephi(emat, phi, precision):
    f = jax.vmap(
        lambda u, v: jnp.matmul(u, v, precision=precision),
        in_axes=(2, None),
        out_axes=2,
    )
    fuv = f(phi, emat)
