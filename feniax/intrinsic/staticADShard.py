...
def main_10g11_1_1(
    ...
    def _fshard(inputs):
        ...
        sol_dict = staticShard.main_10g11_1s(inputs,
                                             t_loads,
                                             q0=q0,
                                             config=config,
                                             args=args1)
    ...
    obj_full = jnp.mean(obj0[0], axis=0)
    obj = obj_full[jnp.ix_(jnp.array([-1]),
                           jnp.array(obj_args.components),
                           jnp.array(obj_args.nodes))]
    #jax.debug.breakpoint()
    #TODO: generalise
    # obj = jnp.mean(obj0[0])
...
def main_10g11_3_1(
    ...
    obj0, f_out0  = jax.pmap(_fshard, axis_name='x')(inputs_shard_reshaped)
    obj_full = jnp.mean(obj0[0], axis=0)
    obj = obj_full[jnp.ix_(jnp.array(obj_args.t),
                           jnp.array(obj_args.components),
                           jnp.array(obj_args.nodes))]
    #jax.debug.breakpoint()
    #TODO: generalise
