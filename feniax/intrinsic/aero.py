...
class AeroRoger(ModalAero):
    def __init__(self, system: intrinsicmodal.Dsystem, sol: solution.IntrinsicSolution):
        self.sys = system
    ...
    def save_sol(self):
        self.sol.add_container(
            "ModalAeroRoger", label="_" + self.sys.name, **self.container
        )
        ...
        for k in container_entries:
            ...
            try:
                if int(k[-1]) == 0:
                    ...
                    self.container[f"{k}hat"] = (
                        self.c_ref * self.rho_inf * self.u_inf / 4 * v
                    )
                elif int(k[-1]) == 2:
                    self.container[f"{k}hat"] = self.c_ref**2 * self.rho_inf / 8 * v
                elif int(k[-1]) == 3:
                    self.container[f"{k}hat"] = self.q_inf * v
            except ValueError:
                continue
        if "A2hat" in self.container.keys():
            A2hat = jnp.eye(len(self.container["A2hat"])) - self.container["A2hat"]
