...
@jax.jit
def contraction_gamma3(
    gamma2: jnp.ndarray, q1: jnp.ndarray, q2: jnp.ndarray
) -> jnp.ndarray:
    res = jnp.einsum("jik,jk->i", gamma2, jnp.tensordot(q1, q2, axes=0))
    return res


@jax.jit
def f_12(omega, gamma1, gamma2, q1, q2):
    F1 = omega * q2 - contraction_gamma1(gamma1, q1) - contraction_gamma2(gamma2, q2)
...
def dq_000001(q, *args):
    (omega, phi1, x, force_follower, t) = args[0]
    F = omega * q
    F += xloads.eta_000001(t, phi1, x, force_follower)
    return F


def dq_001001(q, *args):
    (gamma2, omega, phi1, x, force_follower, t) = args[0]
    F = omega * q - contraction_gamma2(gamma2, q)
    F += xloads.eta_001001(t, phi1, x, force_follower)
    return F
...
# @jax.jit
def dq_0011(q, *args):
    (gamma2, omega, u_inf, rho_inf, qalpha, A0, C0) = args[0]
    q0 = -q / omega
    F = omega * q - contraction_gamma2(gamma2, q)
    F += xloads.eta_0011(q0, qalpha, u_inf, rho_inf, A0, C0)
    return F
...
def dq_101001(t, q, *args):
    (gamma1, gamma2, omega, phi1, x, force_follower, states) = args[0]

    q1 = q[states["q1"]]
    q2 = q[states["q2"]]
    eta = xloads.eta_101001(t, phi1, x, force_follower)
    F1, F2 = f_12(omega, gamma1, gamma2, q1, q2)
    F1 += eta
    F = jnp.hstack([F1, F2])
    return F
...
def dq_100001(t, q, *args):
    (omega, phi1, x, force_follower, states) = args[0]
    q1 = q[states["q1"]]
    q2 = q[states["q2"]]
    eta = xloads.eta_100001(t, phi1, x, force_follower)
    F1 = omega * q2
    F2 = -omega * q1
    F1 += eta
    F = jnp.hstack([F1, F2])
...
def dq_10101(t, q, *args):
    ...
    def _dq_10101(t, q1, q2):
        ...
        eta = xloads.eta_10101(t, phi1l, x, force_dead, Rab)
        F1, F2 = f_12(omega, gamma1, gamma2, q1, q2)
        F1 += eta
        F = jnp.hstack([F1, F2])
        return F
...
if __name__ == "__main__":
    ...
    @jax.jit
    def q_static_ein(q2, omega, gamma2, eta=0.0):
        res = omega * q2
        res += jnp.einsum("ijk,jk->i", gamma2, jnp.tensordot(q2, q2, axes=0))
        # res += eta(q2)
    ...
    @jax.jit
    def q_static_td(q2, omega, gamma2, eta=0.0):
        res = omega * q2
        res += jnp.tensordot(
            gamma2, jnp.tensordot(q2, q2, axes=0), axes=([1, 2], [0, 1])
        )
        # res += eta(q2)

        return res

    def q_static_np(q2, omega, gamma2, eta=0.0):
        res = omega * q2
        res += np.tensordot(gamma2, np.tensordot(q2, q2, axes=0), axes=([1, 2], [0, 1]))
        # res += eta(q2)

        return res

    def q_static_for(q2, omega, gamma2, eta=0.0):
        nm = len(q2)
        res = np.zeros(nm)
        for i in range(nm):
            res[i] = omega[i] * q2[i]
    ...
    for i in range(NUM_ITER):
        ...
        gamma2 = np.arange(NUM_MODES**3, dtype=float).reshape(
    ...
    for i in range(NUM_ITER):
        ...
        gamma2 = jnp.arange(NUM_MODES**3, dtype=float).reshape(
            (NUM_MODES, NUM_MODES, NUM_MODES)
        )
