...
class IntrinsicMultiBodyDriver(Driver, cls_name="intrinsicmultibody"):
    ...
    def _set_systems(self):
        ...
        if hasattr(self._config, "systems"):
            for k, v in self._config.systems.mapper.items():
                ...
                cls_sys = feniax.systems.factory(f"{v.solution}{v.operationalmode}_intrinsicmultibody")
                self.systems[k] = cls_sys(
                    k, v, self._config.fem, self.sol, self._config
                )
                logger.info(f"Initialised {v.solution}{v.operationalmode}_intrinsicmultibody")
            ...
            cls_sys = feniax.systems.factory(
                f"{self._config.system.solution}{self._config.system.operationalmode}_intrinsicmultibody"
            )
            self.systems[name] = cls_sys(
                name, self._config.system, self._config.fem, self.sol, self._config
            )
            logger.info(f"Initialised {self._config.system.solution}{self._config.system.operationalmode}_intrinsicmultibody")
