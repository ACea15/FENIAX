...
- Driver class: initialises all relevant objects in the computation such as the simulation, and the systems objects that will solved in the solution process.
...
** Driver
See [[file:~/projects/FENIAX/feniax/drivers/driver.py::class Driver(ABC):][AbstractClass]] for the contract defining the 
