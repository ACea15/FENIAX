...
that solves the compressible potential equation and is still the de-facto solution among practisioners.


The fluid-structure interaction problem involves the solution of challenging multiphysics phenomena for which various numerical methods have been developed [[cite:&HOU2012]]. 
[[cite:&GEUZAINE2003]]
[[cite:&FARHAT2006]]

While high-fidelity aero-structural nonlinear analysis and optimization have already been demonstrated [[cite:&GRAY2023]], those are mostly restricted to the steady part of the problem. 


Data driven approaches that can also handle unsteady physics have been proposed [urban], however, they are far from being robust for large scale models and accepted in the certification process.
...
This has led to recent efforts geared towards building robust methods that incorporate nonlinear effects to those FEMs, either via stick models constructed for aeroelastic analysis  or using a modal-based approximation cite:&DRACHINSKY2022.
\\
