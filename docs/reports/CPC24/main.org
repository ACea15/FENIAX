        ...
        for mi in [4]:#range(1,6):  # modes
          ...
          for index, row in config.fem.df_grid.iterrows():
            if row.fe_order in list(range(20)):
              ...
              #r_spnl.append(u_spl[li, row.fe_order,:3] + config.fem.X[index])
...
The tolerance in the Newton solver was set to $10^{-6}$ in all cases. A convergence analysis with the number of modes in the solution is presented in [[SPstatic_convergence]].
...
Next we compare the computational times for the various solutions presented in this section in Table [[table:SP_times]]. 
#+NAME: SP_times
...
Table [[table:SP_AD]] shows a very good agreement with a finite differences with an epsilon of $10^{-3}$. 
#+NAME: table:SP_AD
#+CAPTION: Computational times Sail Plane static solution
#+ATTR_LATEX: :width 0.7\textwidth :environment longtable :caption  
