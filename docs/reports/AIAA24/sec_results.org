...
#+begin_src python :results none  :var name=(org-element-property :name (org-element-context)) figfmt=(print Vpics)
  ...
  def plot_Xcomponents(X,  dim1, dim2, labels=None, node=150,
      ...
      dim_dict = {0:'x', 1:'y', 2:'z', 3:r'\theta_x', 4:r'\theta_y', 5:r'\theta_z'}
      fig.update_yaxes(title=r'$\large \hat{X}_{2%s}$' %dim_dict[dim1])
      fig.update_xaxes(title=r'$\large \hat{X}_{2%s}$' %dim_dict[dim2])
      return fig
...
*** Trim flight
The first exercise we setup on this model is the calculation of the trim equilibrium under increasing values of the gravity acceleration as to replicate a pull-up manoeuvre.
...
Next in Fig. [[fig:xrf1_trimaoa]] we show the angle of attack and elevator angles at trim obtained from our nonlinear solver and the linear Nastran solution on the full FE model. Small differences are found streaming from the nonlinearities and the different solution process. For instance, the follower aerodynamic forces mean they are less effective in counterbalancing the weight -which always points downwards- as larger deflections are present. 
...
*** Gust response
Next we look at the gust response of the aircraft under trimmed conditions. Two high intensity gusts are imposed on the 1-g trimmed flight. 
