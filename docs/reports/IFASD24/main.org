...
%which have traditionally been constrained to either linear, frequency domain solutions, or to their nonlinear counterparts but narrower in scope.
...
\\
The computational edge provided by GPUs is shown in a free-flying very flexible structure. It follows an extensive verification by comparison with MSC Nastran full-FE linear and nonlinear structural solutions on a representative aeroplane model.
...
#+begin_src python :results none  :var name=(org-element-property :name (org-element-context)) figfmt=(print Vpics)
  ...
  def wsp_wingtip(sol_list, dim, labels=None, nast_load=None, axes=None,
                  modes = [5, 15, 30, 50, 100],scale = 100./28.8):
...
In this section we briefly describe the backbone theory of the proposed methods for nonlinear aeroelastic modelling as continuation of the work in [[cite:&CEA2021b;&CEA2023]]. A summary of implementation details and computational algorithms are presented next.
...
** Representative aeroplane model: structural verification
...
\\
The aircraft’s main wing is composed of wing surfaces, rear and front spars, wing box and ribs with composite materials employed in the construction. Flexible tail and rear stabiliser are rigidly attached to the wing, as  shown in Fig. [[fig:SailPlane2]]. This aircraft was first shown in [[cite:&CEA2021a]] and is a good test case as it is not very complex yet representative of aircraft FE models and it is available open source.  

#+NAME: fig:SailPlane2
#+CAPTION: Representative aeroplane structural FE model
#+ATTR_LATEX: :width 0.7\textwidth 
...
The computations in this section were carried out on a standard CPU with a i7-6700 processor. 
...
Table [[table:SP_AD]] shows a very good agreement against finite-differences (FD) with an epsilon of $10^{-3}$. Note how the derivative at each of the marked points corresponds approximately to the slope in the graph at those very points. And the biggest slope occurs precisely in between \(tau\) of 4 and 5 when the prescribed loading undergoes the biggest change from 300 to 400 KN.
...
The increase in the internal loading, $f(\alpha)$, above the linear correlation with $\alpha$ is a consequence of very high frequency dynamics being excited after the ramping load is suddenly released. In fact in the z-component of the wing-tip evolution in Fig. [[fig:wsp_adz]] we can see a maximum tip displacement of 4.36m, 7.91m and 10.83m, for $\alpha = 0.5, 1, 1.5$ i.e smaller than the proportional linear response. On the contrary, in Fig. [[fig:wsp_adx2]] the evolution of the root loads show a response with much higher frequencies and the maximum occurs in the free dynamical response of the wing, which is higher as we increase $\alpha$.
#+NAME: WSP_adz
#+begin_src python :results value file  :var name=(org-element-property :name (org-element-context))
  ...
  fig, figname = fig_out(name, update_layout=dict(legend=dict(x=0.13, y=0.3,
      font=dict(size= 16))))(wsp_wingtip)(sol_wspz, dim=2, scale=1, modes=["0.5","1","1.5"], axes=[[0,10],None])
  figname
#+end_src

#+NAME: fig:wsp_adz
#+ATTR_LATEX: :width 0.55\textwidth 
#+CAPTION: Span-normalised wing-tip z-displacement for the various load profiles
#+RESULTS: WSP_adz
...
#+NAME: fig:wsp_adx2
#+ATTR_LATEX: :width 0.55\textwidth 
#+CAPTION: Wing root loads, z-component 
#+RESULTS: WSP_adx2
[[file:figs/WSP_adx2.png]]

\newpage
** Industrial-level case: full-aircraft gust response
...
Similarly to the examples above, we now validate the AD implementation for the nonlinear aeroelastic response to the gust $G1$. The sensitivity of the six components of the wing root loads are computed with respect to the gust parameters $w_g$ and $L_g$, and the flow parameter $\rho_{\inf}$. The results are presented in [[table:XRF1_AD]]. A very good agreement with the finite differences is found with $\epsilon=10^{-4}$.
