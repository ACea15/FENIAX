...
\SetKwFunction{FStrain}{integrate_strains}
 
% Write Function with word ``Function''
  \SetKwProg{Fn}{Function}{:}{}
  \Fn{\FStrain {carry, x}}{
\texttt{
fRab $\longleftarrow$ vmap(lambda Rab_i, H0_i: Rab_i @ H0_i)  \;
fra $\longleftarrow$ vmap(lambda ra_i, Rab_i, H1_i, \gamma_i: ra_i + Rab_i @ H1_i @ ([1,0,0]  + \gamma_i))  \;
strain = x[:, 3]  # 3xNt  \;
kappa = x[:, 4]  # 3xNt  \;
Ipsi = kappa * ds  # 3xNt \;
Itheta = jnp.linalg.norm(Ipsi, axis=0) # Nt \;
Rab_carry = carry[:3]  # 3x3xNt    \;
ra_carry = carry[3]  # 3xNt \;
Rab = fRab(Rab_carry, H0()) \;
ra = fra(ra_carry, Rab_carry, H1(), strain) \;
y = jnp.concatenate([Rab, ra_reshaped], axis=0) #4x3xNt \;
carry = jnp.concatenate([Rab, ra_reshaped], axis=0) \;
\BlankLine
}
        \KwRet carry, y \;
  }
