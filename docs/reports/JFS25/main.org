...
#+begin_abstract
This paper demonstrates a new solution for large scale computations of aircraft-sizing loads. A geometrically nonlinear aeroelastic description has been extended with concurrent capabilities to tackle efficiently the very demanding loading scenarios in the design process. The software can be deployed on modern hardware architectures and has been benchmarked on CPUs and GPUs. While keeping computational times in the order of seconds, the presented approach enables the construction of load envelopes corresponding to static and dynamic cases, seamless integration with full-aircraft models from industry, and uncertainty quantification analysis via massively-parallel Montecarlo simulations, among others. Furthermore, differentiation of both concurrent computations and aeroelastic loads is achieved via JAX library automatic differentiation engine and collective primitives. 
...
#+NAME: jac_ediff
#+begin_src python :results value file  :var name=(org-element-property :name (org-element-context))
  fig, figname = plot_jacediff2(name, mc1_eps, jac_ediff)
  figname
#+end_src
#+NAME: fig:jac_ediff
...
#+begin_src python :results value file  :var name=(org-element-property :name (org-element-context))
  ...
  Ma_plot = jac_fem['Ma'][0,f_index,0,
                jnp.arange(m_start+n_index-shift, m_end+n_index-shift, jump)[:, None],
                jnp.arange(m_start+n_index, m_end+n_index, jump)]
...
#+begin_src python :results value file  :var name=(org-element-property :name (org-element-context))
  ...
  n_index = 4
  jump = 1
  fig, figname = fig_out(name)(plot_jacfemold)(jnp.abs(jac_fem['Ma'][0,f_index,0, m_start+n_index-4:m_end+n_index-4:jump, m_start+n_index:m_end+n_index:jump])/normalisation,
                                            xlabel="i-component",
                                            ylabel="j-component"
                                            )
  figname
