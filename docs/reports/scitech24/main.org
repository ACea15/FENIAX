...
* Run models :noexport:
:PROPERTIES:
:header-args: :var name=(org-element-property :name (org-element-context)) :mkdirp yes :eval (print Veval_blocks) :session py1 :noweb yes
:END:
** ArgyrisBeam
*** Run
:PROPERTIES:
...
#+begin_src python : :exports results :results value file
  ...
  config_simo45f = configuration.Config.from_file("./Simo45Dead/config.yaml")
  icomp = putils.IntrinsicStructComponent(config_simo45f.fem)
  #icomp.add_solution(config_simo45f.fem.X.T)
