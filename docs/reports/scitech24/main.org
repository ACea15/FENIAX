# #+TITLE: A Geometrically Nonlinear Aeroelastic Framework for Real-Time Simulations on Modern Hardware Architectures
...
* Run models :noexport:
:PROPERTIES:
:header-args: :var name=(org-element-property :name (org-element-context)) :mkdirp yes :eval no-export
:END:
...
#+begin_src python :session py1 :results none
  ...
  inp.systems.sett.s1.aero.gust_settings.panels_dihedral = jnp.load(f"{xrf1_folder}/AERO/Dihedral.npy")
...
*** 1
#+NAME: Gl1i1c0u0r0m0
#+begin_src python :session py1 :tangle ./results/run_Gl1i1c0u0r0m0.py :mkdirp yes :noweb yes : results none
  <<PYTHONMODULES>>
  <<GustInput>>
...
#+NAME: SHELL_Gl1i1c0u0r0m0
#+begin_src bash :session zsh1 :shebang #!/usr/bin/zsh :tangle ./results/run_Gl1i1c0u0r0m0.sh :dir ./results/  :results output :async yes
  #source ~/.zshrc
  conda activate fem4inas
  python run_Gl2i1c0u0r0m0.py
#+end_src

*** 2
#+NAME: Gl1i1c1u0r0m0
#+begin_src python :session py1 :tangle ./results/run_Gl2i1c0u0r0m0.py :noweb yes :results none
  <<PYTHONMODULES>>
  <<GustInput>>
  inp.driver.sol_path = pathlib.Path(
      f"./{name}")
...
*** 4
#+NAME: Gl1i1c1u0r0m1
#+begin_src python :session py1 :tangle ./results/run_Gl1i1c0u0r0m1.py :noweb yes :results none
  <<PYTHONMODULES>>
  <<GustInput>>
  inp.driver.sol_path = pathlib.Path(
      f"./{name}")
...
#+begin_src python :session py1 :results value file :exports results
  gscale = 100./33.977
  figname = "figs/XRF1Gust1.png"
  sol_g1 = solution.IntrinsicReader("./Gl2i1c0u0r0m0")
  x, y = putils.pickIntrinsic2D(sol_g1.data.dynamicsystem_s1.t,
                                sol_g1.data.dynamicsystem_s1.ra,
  ...
  fig = uplotly.lines2d(t111[1], u111[1,:,-1, 2]*0.01*gscale, fig,
                        dict(name="NASTRAN",
...
#+RESULTS: Gust1Plot
[[file:figs/XRF1Gust1.png]]

#+NAME: Gust3Plot_x
#+begin_src python :session py1 :results value file :exports results

  figname = f"figs/{name}.png"
  sol_g3 = solution.IntrinsicReader("./Gl1i1c0u0r0m1")
...
#+begin_src python :session py1 :results value file :exports results
        ...
        figname = f"figs/{name}.png"
        sol_g3 = solution.IntrinsicReader("./Gl1i1c1u0r0m1")
        x, y = putils.pickIntrinsic2D(sol_g3.data.dynamicsystem_s1.t,
                                      sol_g3.data.dynamicsystem_s1.ra,
                                      fixaxis2=dict(node=150, dim=0))
...
#+begin_src python :session py1 :results value file :exports results
  ...
  figname = f"figs/{name}.png"
  sol_x1 = solution.IntrinsicReader("./Gl1i1c0u0r0m1")
...
*** Follower forces
#+NAME: Simo45Follower
#+begin_src python :session py1 :tangle ./results/run_Simo45Follower.py :mkdirp yes :noweb yes :var name=(org-element-property :name (org-element-context)) :results none :exports results
  <<PYTHONMODULES>>
  <<Simo45Beam>>
  inp.driver.sol_path = pathlib.Path(
      f"./{name}")
...
#+begin_src python :session py1 :tangle ./results/plot_Simo45Follower.py :results value file :exports results
  import importlib
  importlib.reload(putils)
  importlib.reload(uplotly)
  figname = "figs/s45follower.png"
  sol_s45f = solution.IntrinsicReader("./results/Simo45Follower")
  ...
  annotations.append(dict(#xref='paper',
      ...
      font=dict(family='Arial',
                size=12),
      showarrow=False))
  loads = [f"Load: {li} N" for li in inp.systems.sett.s1.xloads.follower_interpolation[0][1:]]
  for i in range(11):
...
#+NAME: SHELL_Simo45Follower
#+begin_src bash :tangle ./results/run_Simo45Follower.sh :dir ./results/  :results output :shebang #!/usr/bin/zsh :exports results
  source ~/.zshrc
  conda activate fem4inas
  python run_Simo45Follower.py
#+end_src

#+NAME: Simoverificationfollower
#+begin_src python :session py1 :results value file :exports results
    figname = f"figs/{name}.png"
    f = inp.systems.sett.s1.xloads.follower_interpolation[0][1:]
...
*** Dead forces
#+NAME: Simo45Dead
#+begin_src python :session py1 :tangle ./results/run_Simo45Dead.py :mkdirp yes :noweb yes :var name=(org-element-property :name (org-element-context))
  <<PYTHONMODULES>>
  <<Simo45Beam>>
  inp.driver.sol_path = pathlib.Path(
      f"./{name}")
...
#+NAME: SHELL_Simo45Dead
#+begin_src bash :tangle ./results/run_Simo45Dead.sh :dir ./results/  :results output :shebang #!/usr/bin/zsh
  source ~/.zshrc
  conda activate fem4inas
  python run_Simo45Dead.py
#+end_src

#+NAME: Simo45DeadPlot
#+begin_src python :session py1 :tangle ./results/plot_Simo45Follower.py :exports results :eval yes :results value file 
  import importlib
  importlib.reload(putils)
  importlib.reload(uplotly)
...
#+NAME: ArgyrisBeam
#+begin_src python :session py1 :tangle ./results/run_ArgyrisBeam.py :mkdirp yes :noweb yes :var name=(org-element-property :name (org-element-context)) :results none
...
#+NAME: SHELL_ArgyrisBeam
#+begin_src bash :tangle ./results/run_ArgyrisBeam.sh :dir ./results/  :results output :shebang #!/usr/bin/zsh
  source ~/.zshrc
  conda activate fem4inas
  python run_ArgyrisBeam.py
#+end_src
*** Plot
...
#+RESULTS: wingSP_y
[[file:figs/wingSP_y.pdf]]


* Run diagrams :noexport:
:PROPERTIES:
...
\begin{figure}[h!]
 \centering
 \subfigure[Beam evolution]{\includegraphics[width=0.49\textwidth]{figs/s45followerformat2.png}}
\subfigure[Tip displacement]{\raisebox{15mm}{\includegraphics[width=0.499\textwidth]{figs/Simoverificationfollower.png}}}%\label{fig:simo_shapesFoll}
\caption{45-deg bend cantilever deformations}\label{fig:simo45}
\end{figure}
#+end_export
...
#+begin_export latex
 \begin{figure}[h!]
 \centering
 \subfigure[Front view]{\includegraphics[width=0.9\textwidth]{./figs/3Dstatic-frontview.pdf}}
 \subfigure[Side view]{\includegraphics[width=0.9\textwidth]{./figs/3Dstatic-sideview.pdf}}
 \caption{3D structural configuration for wing-tip follower loads}\label{fig:sailplane3Dstatic}
 \end{figure}
#+end_export
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{./figs/wingSP11}
...
\begin{figure}[th!]
\centering
\includegraphics[width=1.\textwidth]{./figs/xrf1-model.pdf}
...
#+begin_export latex
 \begin{figure}[h!]
 \centering
 \subfigure[$\alpha_1$ previous]{\includegraphics[width=0.49\textwidth]{./figs/XRF1Plot_alpha1old.png}}
 \subfigure[$\alpha_1$ current]{\includegraphics[width=0.49\textwidth]{./figs/XRF1Plot_alpha1.png}}
 \subfigure[$\alpha_2$ previous]{\includegraphics[width=0.49\textwidth]{./figs/XRF1Plot_alpha2old.png}}
 \subfigure[$\alpha_2$ current]{\includegraphics[width=0.49\textwidth]{./figs/XRF1Plot_alpha2.png}}
 \caption{Residual $\alpha_1$ and $\alpha_2$ in previous and current implementation}\label{fig:xrf1_alphas}
 \end{figure}
#+end_export
...
#+begin_export latex
 \begin{figure}[h!]
 \centering
 \subfigure[z-component]{\includegraphics[width=0.7\textwidth]{./figs/Gust3Plot_z.png}}
 \subfigure[x-component]{\includegraphics[width=0.49\textwidth]{./figs/Gust3Plot_x.png}}
 \subfigure[y-component]{\includegraphics[width=0.49\textwidth]{./figs/Gust3Plot_y.png}}
 \caption{3D structural configuration for wing-tip follower loads}\label{fig:gust001}
 \end{figure}
#+end_export
...
#+begin_export latex
 \begin{figure}[h!]
 \centering
 \subfigure[z-component]{\includegraphics[width=0.7\textwidth]{./figs/Gust4Plot_z.png}}
 \subfigure[x-component]{\includegraphics[width=0.49\textwidth]{./figs/Gust4Plot_x.png}}
 \subfigure[y-component]{\includegraphics[width=0.49\textwidth]{./figs/Gust4Plot_y.png}}
 \caption{3D structural configuration for wing-tip follower loads}\label{fig:gust2}
 \end{figure}
#+end_export
...
In the immediate future two targets are foreseen with the already built implementation: the first a further assessment of the computational gains by running the examples presented here on GPUs; the second, and more important, the propagation of derivatives in the solution process via the Algorithmic Differentiation tool embedded in JAX. This will complete a fully differentiated aeroelastic framework that can run very efficient in modern software architectures while enhancing traditional FE models that can be very complex by construction but lack the physics of geometrically nonlinear effects.    
