...
import fem4inas.plotools.nastranvtk.vtu_writer as vtu_writer
import fem4inas.plotools.nastranvtk.vtm_writer as vtm_writer
import fem4inas.plotools.nastranvtk.common as common
import fem4inas.plotools.nastranvtk.result_prep as result_prep
...
def write_vtk_vis(filename,fem,onlypts):
    ...
    for i,node in enumerate(nodes):
        ...
        if numpy.any(numpy.isnan(nodepos)) or numpy.any(numpy.isinf(nodepos)):
            print("Inf or nan: %10.3f %10.3f %10.3f" % (nodepos[0],nodepos[1],nodepos[2]))
            farawayNodes[i] = 1
            
        if not pointInBox(nodepos[0],nodepos[1],nodepos[2],bbox[0],bbox[1],bbox[2],bbox[3],bbox[4],bbox[5]):
            farawayNodes[i] = 1
            print("Node Coords: %10.3f %10.3f %10.3f" % (nodepos[0],nodepos[1],nodepos[2]))
...
def run(bdfname,op2name,vtkname,onlypts,punch=False,modes2keep=None,fileformat='binary',xref=True):
    ...
    elementsToWrite = ["CBEAM","RBE1","RBE3","RBE2","RBAR","RBAR1","CONM2","CQUAD4","CTRIA3","CBAR","CBUSH","PLOTEL"]
    ...
    for elemName in elementsToWrite:
        fname = vtu_writer.writeVTU(vtudir,fem,op2,elemName,nodes,nodeIDs,fileformat,modes2keep)
...
if __name__ == "__main__":  
    ...
    parser = argparse.ArgumentParser(description='Converts a NASTRAN .bdf file into a vtk file for visualisation in Paraview')
    subparsers = parser.add_subparsers()
    parser = subparsers.add_parser('Test',help='test')
    parser.add_argument('filenames',type=str,nargs='+', help='The first argument is the name of the bdf-file, the second argument is an optional op2-file (put PARAM,POST,-1 in your bdf to get nastran to write this file out)')
    parser.add_argument('--onlypts',action='store_true',default=False, help='Read only points -- time saving relative to reading everything')
    parser.add_argument('-o','--output',type=str,default='', help='Output vtk filename')
    parser.add_argument('-fmt','--fileformat',type=str,default='binary', help='Change vtk output datatype to ascii setting this to "ascii"')
    parser.add_argument('-p','--punch',action='store_true',default=False,help='Is this a punch card, i.e. a partial FEM without the header section? if so use this. ')
    parser.add_argument('-m','--modes2keep',type=str,default=None,help='Limit the number of modes in the output to this range ')
    ...
    if len(args.filenames) == 2:
        ...
        raise IOError("Specify only one bdf and (optionally) one op2. You specified: %s" % str(args.filenames) )
    ...
    run(bdfname,op2name,vtkname,args.onlypts,args.punch,args.modes2keep,args.fileformat)
