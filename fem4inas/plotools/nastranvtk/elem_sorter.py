...
def sort_spider_element(fem,elemName,elems,nodes,nodeIDs):
    ...
    elemIDs = numpy.zeros((nelems,),dtype=numpy.int_) # This references the actual elements not the extras created here.
...
def sort_elems(fem,elemName,elems,nodes,nodeIDs):
    ...
    if elemName in ["RBE3","RBE2"]:
        nelems,elemtable,node_in_group,nnodes_in_group,elemIDs = sort_spider_element(fem,elemName,elems,nodes,nodeIDs)
    elif elemName in ["RBE1"]:
        nelems,elemtable,node_in_group,nnodes_in_group,elemIDs = sort_super_spider_element(fem,elemName,elems,nodes,nodeIDs)
    else:
        # Default -- assume anything here is a normal element -- worth at try at least.
        nelems,elemtable,node_in_group,nnodes_in_group,elemIDs = sort_normal_element(fem,elemName,elems,nodes,nodeIDs)
