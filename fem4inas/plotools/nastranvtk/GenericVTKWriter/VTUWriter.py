...
import fem4inas.plotools.nastranvtk.FSUtils as FSUtils


def WriteVTU(vtufilename,points,connectivity,pointData={},cellData={},surface=True,datatype="string"):
    ...
    # Root element
    xvtufile = ET.Element('VTKFile',{"byte_order":"LittleEndian",
    ...
    xpoint_chords = ET.Element("DataArray",{"type":"Float64",
                                            "format":datatype,
    ...
    for inode in range(nnodes):
        ...
        if datatype=="binary":
            ...
            string = string + ''.join([repr(nodepos[0]),' ',repr(nodepos[1]),' ',repr(nodepos[2]),' '])
    ...
    for var in sorted(pointData):
        ...
        if pointData[var].shape[0] != nnodes:
            raise ValueError("Point Data variable %s is the wrong length. Number of nodes: %i, length of variable: %i" % (var,nnodes,pointData[var].shape[0]))
        ...
        if len(pointData[var].shape) == 1:
            ...
            raise ValueError("Point Data variable %s is not 1 or 2 dimensional. len(pointData[var].shape) = %i" % (var,len(pointData[var].shape)))

        if ncomponents not in [1,3]:
            raise ValueError("Not sure if number of compnents can be anything other than 1 or 3 (scalar or vector).")
        ...
        if np.issubdtype(arrayDataType,int):
            ...
            raise RuntimeError("Not sure what numpy.dtype: %s translates to in vtk" % (str(arrayDataType)))

        xpointvar = ET.Element("DataArray",{"type":vtkDataType,
    ...
    xcell_connectivity = ET.Element("DataArray",{"type":"Int32",
                                                 "format":datatype,
                                                 "Name":"connectivity"})
    ...
    xcell_offsets = ET.Element("DataArray",{"type":"Int32",
                                            "format":datatype,
                                            "Name":"offsets"})
    ...
    # Element Types
    xcell_types = ET.Element("DataArray",{"type":"Int32",
                                            "format":datatype,
                                            "Name":"types"})

    # Mapping to VTK cell types based on the number of nodes
    if surface: # Surface mesh
        elementTypeMapping = {1:1, # Point
                              2:3, # Line
                              3:5, # Triangle
                              4:9, # Quad
                              }
    else: # Volume mesh
        elementTypeMapping = {4:10, # Tetra
                              8:12, # Hexa
                              6:13, # Prism
                              5:13, # Pyramid
                              }
    ...
    for i in range(len(connectivity)):
        try:
            ...
            raise KeyError("Type for cells with %i nodes not defined yet" % len(connectivity[i]))
    ...
    for var in sorted(cellData):
        ...
        if cellData[var].shape[0] != nelems:
            raise ValueError("Cell Data variable %s is the wrong length. Number of nodes: %i, length of variable: %i" % (var,nnodes,cellData[var].shape[0]))
        ...
        if len(cellData[var].shape) == 1:
            ...
            raise ValueError("Cell Data variable %s is not 1 or 2 dimensional. len(cellData[var].shape) = %i" % (var,len(cellData[var].shape)))
        ...
        xcellvar = ET.Element("DataArray",{"type":vtkDataType,
                                           "NumberOfComponents":str(ncomponents),
                                           "format":datatype,
                                           "Name":var})
        #string = str()
