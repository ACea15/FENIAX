...
for i in range(NumBeams):
    ...
    if BeamSeg[i].nx == 1:
      ...
      for k in range(BeamSeg[i].nx-1):
          ...
          if   i not in BeamsClamped and i!=0 and k==0:
              GA=X2G[tuple(BeamSeg[BeamConn[i]].NodeX[-1])];GB=X2G[tuple(BeamSeg[i].NodeX[k])]
              cbeam=['CBEAM',EID,PID,GA,GB,X1,X2,X3]
              mesh.add_card(cbeam,'CBEAM')
              EID=EID+1
...
if sol is not '103':
  ...
  Fd=[[0.,0.,1.]]
  GF=[X2G[tuple(BeamSeg[-1].NodeX[-1])]]
  for i in range(numloads):
    sid=2*(i+1)+1
    load1=['LOAD',sid,1.]
    for j in range(numForce):
     ...
     G= GF[j]
     F= Fl[j][i]
     force1=['FORCE',sid,G,0,F,Fd[j][0],Fd[j][1],Fd[j][2]]
...
for i in range(1):
  ...
  folder1=folder1+'Nx%s' % int(pc)


if not os.path.exists(folder0+'/'+folder1):
  os.makedirs(folder0+'/'+folder1)


femname='S_0'
   
mesh.write_bdf(folder0+'/'+folder1+'/'+femname+ '.bdf')
