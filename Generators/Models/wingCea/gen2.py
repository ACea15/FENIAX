...
m1 = mgen.Model('wing_inp2',mesh1)
wm1 = mgen.WingBox_model('wing_inp2',m1)
...
m1.write_model(model='/m2.bdf')
######################################################################

############################Model for Guyan reduction################
mesh1a=BDF(debug=True,log=None)
m1a = mgen.Model('wing_inp2',mesh1a)
wm1a = mgen.WingBox_model('wing_inp2',m1a)
...
m1a.write_model(model='/m2a.bdf')
m1a.write_includes(['m2.bdf'],model='/m2a.bdf')
######################################################################

############################Sol 103###############################3
mesh2=BDF(debug=True,log=None)
m2 = mgen.Model('wing_inp2',mesh2)
wm2 = mgen.WingBox_model('wing_inp2',m2)
...
m2.write_model(model='/m2_103.bdf')
m2.write_includes(['m2.bdf'],model='/m2_103.bdf')
###################################################################

############################Sol 103 DMAP for M&K###############################3
mesh2d=BDF(debug=True,log=None)
m2d = mgen.Model('wing_inp2',mesh2d)
wm2d = mgen.WingBox_model('wing_pch2',m2d)
am2d = mgen.Aerodynamic_model('wing_inp2',m2d)
...
m2d.write_includes(['m2.bdf'],model=model)
#if run_nastran:
...
m2p = mgen.Model('wing_pch2',mesh2p)
wm2p = mgen.WingBox_model('wing_pch2',m2p)
...
m2p.write_model(model='/m2_103p.bdf')
m2p.write_includes(['m2.bdf'],model='/m2_103p.bdf')
###################################################################

########################### 103 sol Guyan condensation########################################
mesh2a=BDF(debug=True,log=None)
m2a = mgen.Model('wing_inp2',mesh2a)
wm2a = mgen.WingBox_model('wing_inp2',m2a)
...
m2a.write_model(model='/m2a_103.bdf')
m2.write_includes(['m2a.bdf'],model='/m2a_103.bdf')

#EXTSEOUT(STIFFNESS ASMBULK EXTID = 10 MATOP4=18)

###################### Sol 400 tip force#############################################
mesh3=BDF(debug=True,log=None)
m3 = mgen.Model('force_inp2',mesh3)
wm3 = mgen.WingBox_model('force_inp2',m3)
...
m3.write_model(model='/m2_400mg.bdf')
m3.write_includes(['m2.bdf'],model='/m2_400mg.bdf')

####################### Sol 400 pressure force############################################
mesh4=BDF(debug=True,log=None)
m4 = mgen.Model('force_inpp2',mesh4)
wm4 = mgen.WingBox_model('force_inpp2',m4)
wm4.run(box_initials=0,box=0,asets=1,aset1=0,mass_asets=0,interpolation_elements=1,constraints=0,initial_conditions=0,forces=1)
m4.executive_control_deck()
m4.case_control_deck(setx=range(500001,500001+30))
m4.write_model(model='/m2_400p.bdf')
m4.write_includes(['m2.bdf'],model='/m2_400p.bdf')
###################################################################
