...
if run_model:
    ...
    m1 = mgen.Model('beam_inp',mesh1)
    bm1 = mgen.Beam_model('beam_inp',m1)
    bm1.run(masses=1,constraints=1,initials=0,forces=0)
    m1.write_model(model='/model.bdf')
    node_id2position={}
    nids=m1.mesh.node_ids[1:]
    for ni in nids:
...
if run_AICs:
    ...
    femmb.mat2dmig(nids,[m3.path+'/Ka.op4',m3.path+'/Ma.op4'],Mread=['KAA','MAA'],Msave=['KAAX','MAAX'],write_dmig=m3.path+'/dmig.bdf',aset_coord=node_id2position)
    BeamSeg = gridorder.bdf_order_nodes([[nids[0]-2,nids[-1]-2]],m2.path+'/model103.bdf',nodestart=2)
    ...
    for mi in machs:
        ...
        m4 = mgen.Model('beam_inp',mesh4)
        m4.write_modes('HaleX1cx',V.NumModes,rb=0)
        am4= mgen.Aerodynamic_model('aero_inp',m4)
