...
def geometry_def(Grid,NumBeams,BeamConn,start_reading,beam_start,nodeorder_start,node_start,Clamped,ClampX,BeamsClamped,MBbeams=[]):
    ...
    if duplicate:
      for i in range(NumBeams):
        for j in range(len(BeamConn[0][i])):
          #pdb.set_trace()  
          if BeamConn[0][i][j] not in MBbeams:
