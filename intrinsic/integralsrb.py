...
# except IOError:
#from intrinsic.modesrb import Phi0l,Phi1l,Phi1ml,MPhi1l,Phi2l,CPhi2xl,Omega,Phi1,Phi1m,Phi2,MPhi1

#from pathos.multiprocessing import ProcessingPool as Pool
#Integral to compute intrinsic coefficients. Cycle through nodes.
#================================================================

if V.quadratic_integrals:
    def integral_gammas(BeamSeg,V,Phi1,Phi1l,Phi1m,MPhi1,Phi2,CPhi2):
                ...
                for k3 in range(V.NumModes):
                    ...
                    for i in range(V.NumBeams):
                        for j in range(0,BeamSeg[i].EnumNodes-1):
                            quadterm=np.array([0., 1./12*(Phi1l[i][k1][j,4] - Phi1l[i][k1][j+1,4]), 1./12*(Phi1l[i][k1][j,5] - Phi1l[i][k1][j+1,5]), 0., 0., 0.])*BeamSeg[i].NodeDL[j]
                            if i in V.initialbeams and j==0:
                               if i in V.BeamsClamped:
                                   pass
                               elif i in V.MBbeams:
                                   gamma1 = gamma1 + Phi1[i][k1][j,:].T.dot(intrinsic.functions.L1fun(Phi1[i][k2][j,:].T)).dot(MPhi1[i][k3][j,:])
                               else:
                            ...
                            gamma1 = gamma1 + Phi1[i][k1][j+1,:].T.dot(intrinsic.functions.L1fun(Phi1[i][k2][j+1,:].T)).dot(MPhi1[i][k3][j+1,:])
                            gamma2 = gamma2 + (Phi1m[i][k1][j,:]+quadterm).T.dot(intrinsic.functions.L2fun(Phi2[i][k2][j,:].T)).dot(CPhi2[i][k3][j,:])*BeamSeg[i].NodeDL[j]

                    Gamma1[k1][k2][k3] = gamma1
                    Gamma2[k1][k2][k3] = gamma2
        return Gamma1,Gamma2


else:
    def integral_gammas(BeamSeg,V,Phi1,Phi1m,MPhi1,Phi2,CPhi2):
                ...
                for k3 in range(V.NumModes):
                    ...
                    for i in range(V.NumBeams):
                        for j in range(0,BeamSeg[i].EnumNodes-1):
                            ...
                            gamma2 = gamma2 + Phi1m[i][k1][j,:].T.dot(intrinsic.functions.L2fun(Phi2[i][k2][j,:].T)).dot(CPhi2[i][k3][j,:])*BeamSeg[i].NodeDL[j]
...
def integral_alphas(BeamSeg,V,Phi1,Phi1m,MPhi1,Phi2,CPhi2):
                ...
                for j in range(0,BeamSeg[i].EnumNodes-1):
                    ...
                    if i in V.initialbeams and j==0:
                       if i in V.BeamsClamped:
                           ...
                           alpha1 = alpha1+ Phi1[i][k1][j,:].T.dot(MPhi1[i][k2][j,:])
                       else:
                           if i == V.initialbeams[0]:
                               alpha1 = alpha1+ Phi1[i][k1][j,:].T.dot(MPhi1[i][k2][j,:])
                    elif i in V.MBbeams and j==0:
        ...
        for j in range(0,BeamSeg[i].EnumNodes-1):
                ...
                if i in V.initialbeams and j==0:
                   if i in V.BeamsClamped:
                       ...
                       alpha1 = alpha1+ Phi1y[i][k1][j,:].T.dot(MPhi1y[i][k2][j,:])
                   else:
                       if i == V.initialbeams[0]:
                           alpha1 = alpha1+ Phi1y[i][k1][j,:].T.dot(MPhi1y[i][k2][j,:])
                elif i in V.MBbeams and j==0:
            ...
            for j in range(0,BeamSeg[i].EnumNodes-1):
                ...
                if i in V.initialbeams and j==0:
                   if i in V.BeamsClamped:
                       ...
                       gamma1 = gamma1 + Phi1y[i][k1][j,:].T.dot(intrinsic.functions.L1fun(Phi1y[i][k2][j,:].T)).dot(MPhi1y[i][k3][j,:])
                   else:
                       if i == V.initialbeams[0]:
                           gamma1 = gamma1 + Phi1y[i][k1][j,:].T.dot(intrinsic.functions.L1fun(Phi1y[i][k2][j,:].T)).dot(MPhi1y[i][k3][j,:])
                elif i in V.MBbeams and j==0:
...
if (__name__ == '__main__'):
    ...
    gamma1i,gamma2i=integral_gammas(BeamSeg,V,Phi1,Phi1ml,MPhi1,Phi2l,CPhi2xl)
