...
for i in range(V.NumBeams):
  ...
  if V.Clamped or V.Path4Phi2:
         ...
         totalchildseglist,direction = intrinsic.beam_path.TreeOpt2(intrinsic.beam_path.Treesec2(i,BeamSeg)[0],
                                                                    intrinsic.beam_path.Treesec2(i,BeamSeg)[1])
  ...
  for k in range(V.NumModes):
      ...
      if V.RigidBody_Modes and k<6:
          ...
          for j in range(BeamSeg[i].EnumNodes):
            ...
            if   i!=0 and j==0:
              pass
               ...
               MPhi1[i][k][j] = MdotV[k,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6]
        ...
        for j in range(BeamSeg[i].EnumNodes):
                ...
                if j==0 and  V.Clamped and i in V.BeamsClamped:
                    ...
                    Phi0[i][k][j] = Vreal[k,BeamSeg[inverseconn[i]].NodeOrder[-1]*6:BeamSeg[inverseconn[i]].NodeOrder[-1]*6+6]
                    MPhi1[i][k][j] = Omega[k]*MdotV[k,BeamSeg[inverseconn[i]].NodeOrder[-1]*6:BeamSeg[inverseconn[i]].NodeOrder[-1]*6+6]
               #MPhi1[i][k][j] = Ma[BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6].dot(Vreal[k,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6])
                    MPhi1l[i][k][j] = intrinsic.functions.Rot6(BeamSeg[i].GlobalAxes.T).dot(MPhi1[i][k][j])
                else:
                    #pdb.set_trace()
                    Phi0[i][k][j] =  Vreal[k,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6]
                    MPhi1[i][k][j] = Omega[k]*MdotV[k,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6]
               #MPhi1[i][k][j] = Ma[BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6].dot(Vreal[k,BeamSeg[i].NodeOrder[j]*6:BeamSeg[i].NodeOrder[j]*6+6])
                    MPhi1l[i][k][j] = intrinsic.functions.Rot6(BeamSeg[i].GlobalAxes.T).dot(MPhi1[i][k][j])
                    MPhi1l2[i][k][j] = Omega[k]
      ...
      if V.Check_Phi2 and (not V.Path4Phi2):
          if k > 5:
            ...
            err_total = np.linalg.norm(Phi2_total[i][k])
            #print err
            #pdb.set_trace()
