...
#+begin_src python :session py1 :results none
  machs = [0.8]
  reduced_freqs = np.hstack([np.linspace(1e-5,10, 50), [10.01, 10.02]])
...
#+begin_src python :session py1 :var output="num_modes" :results none
  ...
  num_modes = 20
  chord_panels = dict(wing=10, hstabilizer=10, vstabilizer=10)
  #aero['s_ref'] = 361.6
  #aero['b_ref'] = 58.0
  #aero['X_ref'] = 36.3495
  flutter_id = 9010
...
* GAFs extraction
#+begin_src python :session py1 :results none
  dlm_panels = nasaero.GenFlutter(flutter_id,
                                  density_fact,
                                  mach_fact,
                                  kv_fact,
                                  machs,
...
#+begin_src python :session py1  :results none :noweb yes
  ...
  Qhh = op4.read_op4(f"/home/ac5015/programs/FEM4INAS/examples/SailPlane/NASTRAN/GAFs/Qhh{Mach}-{num_modes}.op4")
