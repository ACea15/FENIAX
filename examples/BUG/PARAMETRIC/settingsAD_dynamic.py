...
inp.driver.sol_path= pathlib.Path("./results_dynamicADfem")
...
inp.system.xloads.follower_points = [[35, 2],
                                     [61, 2]]
inp.system.xloads.x = [0, 4, 4+1e-6, 20]
force=[0.05 * 2e4, 1 * 2e4, 0., 0.]
inp.system.xloads.follower_interpolation = [force,force]
...
inp.system.ad = dict(inputs=dict(Ka=jnp.load("./FEM/Ka.npy"),
                     ...
                     objective_fun="max",
                     objective_var="ra",
                     objective_args=dict(nodes=(35,), components=(2,))
                     )
config =  configuration.Config(inp)
